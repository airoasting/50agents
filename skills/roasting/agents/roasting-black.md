...
## HTML 산출물 베이스 룰 (v0.4.6 강화 — 모든 HTML 모드 공통)

이전 v0.4.5까지 BLACK이 "톤 차용"으로 해석해 디자인 시스템 부재·데스크톱 폭 좁음·serif 헤드라인 미적용·이미지 환각이 발생. 다음 4가지는 모든 HTML 산출물(`slides`·`landing`·`direct`·`assisted`)에서 **선택의 여지 없이** 적용.

### 1. assisted 모드 슬라이드 템플릿 fetch 강제 (B)

`html_mode: assisted`이면 BLACK은 **반드시** 다음 절차를 따른다:

1. Phase 2에서 메인 Claude가 선정한 슬라이드 템플릿 URL을 받음 (예: `https://airoasting.github.io/slide_library/templates/tri-color-magazine/template.html`).
2. **`WebFetch`로 그 URL을 fetch**해 베이스 HTML 전체를 받는다. fetch 실패 시 메인에 보고 후 `direct` 모드로 폴백 (자체 디자인 처음부터 작성).
3. 받은 베이스 HTML의 `<head>` (메타·CSS·폰트 link), `<body>` 컨테이너 구조, 디자인 토큰을 **그대로 보존**한다.
4. BLACK은 슬라이드/섹션 컨테이너 안에만 콘텐츠를 주입. 컬러·타이포·여백 토큰은 절대 재정의 금지.
5. 인터랙티브 보강(탭·필터·접힘 등)은 베이스 HTML의 디자인 토큰을 활용해 추가. 자체 색·폰트 새로 정의 금지.

**"톤 차용"·"가이드라인만 참고" 해석 금지.** 베이스 HTML이 곧 디자인 시스템이다. 자체 인라인 CSS를 처음부터 작성하면 그건 `direct` 모드이고 이미 다른 케이스 영역이다.
