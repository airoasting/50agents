...
- [ ] **(v0.4.13 신설) 영어 동사 직역체 0회.** 3패턴 모두 차단:
  - **(1) break 직역** — "직무가 깨진다 / 모델이 깨진다 / 시스템이 깨진다" 류. 자연: 더는 통하지 않는다 · 멈춘다 · 무너진다 · 흔들린다.
  - **(2) kill 직역** — "가설을 죽인다 / 아이디어를 죽인다 / 옵션을 죽인다" 류. 자연: 흔들다 · 의심하다 · 시험하다 · 골라내다 · 접다.
  - **(3) force/make 사역 직역** — "AI에게 X를 시킨다" 류. 자연: AI로 X해 보다 · AI에게 던져 X받다 · AI가 X하게 두다.
  - **메타 원칙:** 영어 능동 동사(break·kill·force·make·push·drive·leverage)가 떠올랐을 때 한국어 1:1 직역 위험. 한국어 화자는 더 부드럽고 비유적인 동사 풀(흔들다·접다·다듬다·풀다·받다·두다)로 의미를 다시 짠다. `references/anti-patterns/english-verb-direct.md` 자연 표현 대체 사전 참조. v0.4.12 회의실 jargon(영어 imperative → 강한 단음절 동사)과 함께 작동하는 두 번째 직역체 차단 축.
...
### 2. 절대 금지 (v0.4.8 — 환각 룰 회귀 차단, v0.4.15 JS 룰 완화)
...
| `<script src=...>` 외부 스크립트 (슬라이드용) | 의존성·트래킹·analytics 차단. v0.4.15부터는 §5 표준 네비게이션 인라인 블록 *1개*만 허용. |
| §5 표준 네비게이션 블록 외 임의 인라인 `<script>` | 표준 1개만 허용. 인터랙티브 보강은 `<details>`·`:target`·CSS 트랜지션으로. |
...
- [ ] `<script src=` 0회 (외부 스크립트 금지)
- [ ] `<script>` 정확히 1회 (§5 표준 네비게이션 블록만, v0.4.15)
- [ ] `1920px` / `1080px` 0회 (고정 폭 금지)
- [ ] `<div class="deck">` 1회 / `<div class="slide` N회 (N = 슬라이드 수)
- [ ] 첫 슬라이드에 `class="slide active"` (또는 동등 표시 클래스) 존재
- [ ] `.slide.active` CSS 룰 존재
- [ ] `keydown` 이벤트 리스너 존재 (화살표 네비게이션)
- [ ] `#nav-bar` 또는 동등 네비게이션 위젯 존재
- [ ] `max-width: 1200px` 존재
- [ ] `aspect-ratio: 16 / 10` 존재
- [ ] `container-type: inline-size` 존재
- [ ] `cqi` 단위 10회 이상 사용

`slides`·`landing` 모드도 동일. `landing` 모드는 §5 네비게이션 블록 *생략* (스크롤이 의도).

`direct` 모드만 BLACK이 처음부터 자체 HTML 작성 (c73 전용). 네비게이션 룰 적용 안 함.

## 5. 표준 슬라이드 네비게이션 (v0.4.15 신설)

**핵심 원칙: 슬라이드 데크는 키노트/파워포인트처럼 한 장씩 화살표로 넘어가야 한다.** v0.4.14까지는 슬라이드를 세로로 스크롤하는 구조였다. 사용자가 슬라이드 데크를 열었을 때 화살표 키가 안 먹는다고 보고된 패턴이 반복 발생. v0.4.15부터 모든 `html_mode: slides|assisted` 산출물은 다음 표준 블록을 *정확히 1개* 포함한다.

### 5.1 CSS 추가 (베이스 룰 §1 위에 덮어쓰기)

```css
.deck {
  position: relative;
  width: min(96vw, 96vh * 1.6);
  max-width: 1400px;   /* v0.4.15: 풀스크린 발표용으로 확장 */
  aspect-ratio: 16 / 10;
  margin: 0 auto;
}
.slide {
  ...
  /* aspect-ratio·container-type·cqi 등 베이스 룰은 그대로 */
}
.slide.active {
  display: flex;
  opacity: 1;
...
#nav-bar button:hover { background: rgba(128, 128, 128, 0.15); }
#nav-counter { min-width: 5vh; text-align: center; }
#nav-hint {
  position: fixed;
  top: 1.2vh;
  right: 1.6vh;
...
### 5.2 HTML 베이스 (body 끝, `</div>` 데크 닫힘 직후)

```html
<div id="nav-hint">← → 키로 넘기기 · F 키로 전체화면</div>
<div id="nav-bar">
  <button id="nav-prev" aria-label="이전 슬라이드">‹</button>
  <span id="nav-counter">1 / 1</span>
  <button id="nav-next" aria-label="다음 슬라이드">›</button>
</div>
```

첫 슬라이드는 `<div class="slide active">` (또는 동등 클래스), 나머지는 `<div class="slide">`.

### 5.3 표준 네비게이션 스크립트 (정확히 1개, 변형 금지)

```html
<script>
/* v0.4.15 표준 슬라이드 네비게이션 — 변형 금지 */
(function () {
  ...
  document.addEventListener('keydown', function (e) {
    ...
    else if (e.key === 'End') { e.preventDefault(); render(slides.length - 1); }
  ...
  if (np) np.addEventListener('click', prev);
  if (nn) nn.addEventListener('click', next);
  ...
  if (!isNaN(initial) && initial >= 1 && initial <= slides.length) render(initial - 1);
  else render(0);
})();
</script>
```

### 5.4 적용 절차 (WebFetch 베이스 템플릿 후처리)

1. Phase 2에서 슬라이드 템플릿 fetch (`airoasting.github.io/slide_library/templates/{id}/template.html`).
2. 받은 HTML에서 `:root` 토큰·`.deck`·`.slide` 베이스 CSS는 그대로 보존.
3. §5.1 CSS를 베이스 CSS *뒤*에 추가 (later wins).
4. §5.2 HTML 위젯을 `</body>` 직전에 삽입.
5. §5.3 표준 스크립트를 위젯 직후에 삽입.
6. 첫 슬라이드 `<div class="slide ...">` → `<div class="slide active ...">`로 한 번 수정.

### 5.5 적용 제외

- `landing` 모드 (c70): 스크롤이 의도. 네비게이션 블록 *생략*.
- `direct` 모드 (c73): 풀스택 페이지. 적용 안 함.
- Markdown 산출물: HTML이 아니므로 무관.

### 5.6 사용자 노출

Phase 7 완료 푸터에 한 줄 안내 (v0.4.15 신설):

> 조작: ← → 키 (또는 스페이스) · Home/End · F 전체화면 · 클릭 다음 슬라이드
