...
목적: BLACK 산출물의 7종 안티패턴 검출 → 발견 시 BLACK 재작성 (라운드 카운트 안 깎임). 3-strikes 보호.

**검출 룰 7종 (v0.4.5):** `hallucinated-number` · `legal-risk-term` · `missing-gold-hook` · `tone-mismatch` · `vague-cta` · **`unsourced-fact` (신설)** · **`fake-image-url` (신설)**. 각 룰의 `applies_to_categories`로 케이스 카테고리별 적용 여부 결정.
...
| `references/anti-patterns/*.md` | 7종 검출 룰 (`hallucinated-number`, `legal-risk-term`, `missing-gold-hook`, `tone-mismatch`, `vague-cta`, `unsourced-fact`, `fake-image-url`) + BLACK 재작성 지시 | Phase 4 (적용 안티패턴만) |
...
**한국어 전용. 베타 v0.4.6.** 세부 변경 이력은 [`CHANGELOG.md`](../../CHANGELOG.md) 참조.
