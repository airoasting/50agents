...
description: HTML 슬라이드 산출물에서 진짜 베이스 룰 위반. <deck-stage>·deck-stage.js 같은 환각 룰 사용, 1920px 고정 폭, .deck 컨테이너 누락, cqi 단위 누락 등 사용자 화면에서 깨지는 사고 차단 (v0.4.8 신설, v0.4.15 표준 네비게이션 룰 완화)
...
### C. JS 의존 (v0.4.15 — 표준 네비게이션 외 임의 JS 금지)
8. 슬라이드 케이스 (`html_mode: slides|assisted`)에서 다음 중 하나라도 양성이면 위반:
   - `<script src=` 외부 스크립트 로드 (의존성·트래킹·analytics 차단)
   - 표준 네비게이션 블록(`agents/roasting-black.md` §5 "표준 슬라이드 네비게이션")이 아닌 임의 인라인 `<script>` 본문
   - 표준 네비게이션 블록이 *아예 누락*된 슬라이드 데크 (BLACK 작성 의무 누락)

표준 네비게이션 블록 1개만 허용한다. 인터랙티브 보강이 필요해도 별도 `<script>`를 추가하지 말고 `<details>`·`:target`·CSS 트랜지션으로 풀어라.
...
### D. 표준 네비게이션 누락 (v0.4.15 신설)

9. 슬라이드 케이스 (`html_mode: slides|assisted`)에서 다음 중 하나라도 양성이면 위반:
   - `keydown` 이벤트 리스너 누락 (화살표 네비게이션 부재)
   - `.slide.active` CSS 룰 누락 (현재 슬라이드 표시 메커니즘 부재)
   - 첫 슬라이드에 `class="slide active"` (또는 동등 표시) 누락
   - 하단 네비게이션 위젯 (`#nav-bar`) 누락

상세 표준 블록은 `agents/roasting-black.md` §5 "표준 슬라이드 네비게이션 (v0.4.15)" 참조.
...
4. **표준 네비게이션 블록 1개 + 인터랙티브 보강은 무-JS로** (v0.4.15)
   ```html
   <script>/* v0.4.15 표준 슬라이드 네비게이션 */ ...</script>
   <details><summary>액션 3 보기</summary><p>...</p></details>
   ```
   ✓ 화살표·스페이스 네비게이션 + `<details>` 접힘. 임의 JS 추가 금지.
